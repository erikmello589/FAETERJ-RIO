...
function limitaCep (itemCep)
{
  var variavelCep = itemCep.value;
    ...
    itemCep.setAttribute("maxlength", "9");
    if (variavelCep.length == 5) itemCep.value += "-";
}
  ...
  { 
    ...
    { 
      ...
      { 
        ...
        // função criada em tentativa de fazer a remoção de um cadastro específico.
        /*button.addEventListener = ("click", () => {
          bancoDados.transaction((tx) => {
            tx.executeSql(
    ...
    // ativar somente quando desejar limpar o banco de dados armazenado.
    /*bancoDados.transaction(function(tx) {
...
function listarCandidatosNome() {
  enviarRequisicao('listarCandidatosNome.php', 'GET', {})
      .then(response => console.log(response));
}

function listarCandidatosSala() {
  enviarRequisicao('listarCandidatosSala.php', 'GET', {})
      .then(response => console.log(response));
}

// Função para criar um candidato
function enviaCandidato () {
  ...
  enviarRequisicao('incluirCandidato.php', 'POST', dados)
      .then(response => console.log(response));
}